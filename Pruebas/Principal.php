...
include_once '../De comportamiento/Command.php';
...
class Principal {
    ...
    public function probarCommand() {
        $tipos = ["Luz", "Television", "DecodificadorSky"];
        $tipo = $tipos[array_rand($tipos)];


        $control = new ControlRemoto();

        $dispositivo = new $tipo();
        $control->setComando("ON", new EncenderComando($dispositivo));
        $control->setComando("OFF", new ApagarComando($dispositivo));
...
while (true) {
    ...
    switch ($opcion) {
        ...
        case 'h':
            ...
            break;
        case 'i':  // Nuevo caso para Decorator
